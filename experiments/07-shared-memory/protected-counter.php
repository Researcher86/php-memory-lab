...
return new Experiment(
    name: 'shm:counter',
    description: 'a semaphore-protected counter, and what the lock costs',
    supports: ['children', 'iterations'],
    run: static function (Options $options, Output $out): void {
        $out->heading('a shared counter that is always right, and what the lock costs');
        ...
        $increments = $options->iterations(1_000);
        $counterIndex = 1;
        $reportBase = 100;
        ...
        $out->note('a lock turns concurrent processes into a queue. Four children do not finish four times faster - they take turns, and the time they spend waiting is time the CPU has nothing to do on their behalf.');
