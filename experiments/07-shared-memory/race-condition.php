...
return new Experiment(
    name: 'shm:race',
    description: 'lost updates, then a destroyed counter, at 1 to 16 writers',
    supports: ['iterations'],
    run: static function (Options $options, Output $out): void {
        $out->heading('shared memory without a lock: counting how many updates get lost');
         ...
         * still loses them.
         */
        $increments = $options->iterations(500);
        $counterIndex = 1;
        ...
        $out->note('with one child there is nobody to race against, so the unsynchronized column is correct - which is exactly how this class of bug survives testing and shows up under load.');
        $out->note('a few writers lose updates silently: no error, no warning, just a total that does not add up. That is the failure mode a retry cannot fix, because nothing reports a failure to retry.');
        $out->note('enough writers and the counter stops existing. PHP stores each variable in a directory inside the segment and rewrites the entry in place; concurrent writers corrupt that bookkeeping, not just the value. Shared memory does not degrade gracefully - it stops being a data structure.');
        $out->note('the lock costs wall-clock time, growing roughly linearly with the number of writers, and buys the only thing that matters: the protected column always equals the expected one.');
        $out->context();
    },
);
