...
return new Experiment(
    name: 'shm:lifecycle',
    description: 'serialization, size limits, RssShmem and stale segments',
    supports: [],
    run: static function (Options $options, Output $out): void {
        $out->heading('SysV shared memory: what it really stores, and how long it lives');
        ...
        $out->note('shared pages are charged to RssShmem in /proc/self/status - part of RSS, and no part of memory_get_usage(). A process whose PHP memory looks flat can still be holding megabytes this way.');
        ...
        $out->note('a segment survives every process that touched it; only shm_remove() or a reboot takes it away. That is why `ipcs -m` on a long-lived box is full of segments nobody owns any more, and why every test and experiment here cleans up by key rather than by object.');
        $out->context();
    },
);
