...
experiment_start('mmap: a mapping is a promise, not a read');
...
experiment_note('the address space grew by the size of the file and the resident memory did not. Nothing has been read yet.');
experiment_note('note also that a 256.00 MiB mapping succeeded under a 128M memory_limit. The limit counts what the PHP allocator hands out, and a mapping is not that - which is the first of several ways this phase and the next make memory that memory_get_usage() cannot see.');
...
experiment_note('RSS follows what was touched, but not one page at a time: the fault count is far below the page count, and each fault brings in close to a megabyte. The kernel reads ahead and maps whole folios, so a sequential walk pays for one fault per large run rather than one per page.');
experiment_note('which is why the rows with no faults at all still cost time - those pages were already mapped by the fault before them, and reading them is just memory access.');
...
experiment_note('munmap() gave back every page that had been touched in one call - the RSS delta across map, touch and unmap is back to noise. Unlike freed PHP memory, which the allocator keeps, an unmapped region is gone from the process immediately.');
...
experiment_note('the mapping paid for the 16.00 MiB it touched; file_get_contents() paid for all 256.00 MiB, because a PHP string has no way to be partly present - and had to be allowed a larger memory_limit to do it.');
experiment_note('this is why a mapped file is the right shape for a large index, a database page cache or a read-mostly dataset, and the wrong shape for anything that is going to be scanned once from end to end anyway.');
experiment_context();
