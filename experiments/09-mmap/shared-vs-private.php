...
experiment_start('MAP_SHARED against MAP_PRIVATE: the same call, two different machines');
...
foreach ([true, false] as $shared) {
    ...
    \fwrite(STDOUT, \sprintf("  the writer itself reads: %s\n", \rtrim($mapping->read(0, 24))));
    ...
    if ($pid === 0) {
        ...
        \fwrite(STDOUT, \sprintf("  another process reads:   %s\n", \rtrim($reader->read(0, 24))));
...
experiment_note('the shared mapping changed the file for everyone without a single write() call and without a flush. The private one changed nothing outside the process that wrote it - it faulted its own copy of the page, exactly as a forked child does.');
...
if ($pid === 0) {
    ...
    experiment_smaps('  child, reading the same 16.00 MiB ', $smaps->read());
    ...
    experiment_smaps('  child, after writing to all of it  ', $smaps->read());
    \usleep(400_000);
    $child->unmap();
...
\usleep(150_000);
experiment_smaps('  parent, while the child maps it too', $smaps->read());
\pcntl_waitpid($pid, $status);
...
experiment_smaps('  parent, after the child exited     ', $smaps->read());
...
experiment_note('msync() buys durability against a power cut, not visibility to other processes - they are looking at the same page cache this process is writing to, so they were never behind.');
experiment_note('a MAP_SHARED mapping is therefore shared memory with a filename: the same properties as phase 6\'s segment, minus the serialization, plus a lifetime managed by the filesystem instead of by ipcs.');
experiment_context();
