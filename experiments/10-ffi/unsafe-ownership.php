...
return new Experiment(
    name: 'ffi:unsafe',
    description: 'double free, use-after-free and overflow, each in its own child',
    supports: [],
    run: static function (Options $options, Output $out): void {
        $out->heading('native memory ownership: the mistakes, and who reports them');
        ...
        $out->write("\"malloc():\" below come from glibc, not from PHP - the allocator is the\n");
        $out->write("only thing in the stack that has any opinion about these.\n\n");
        ...
        $out->note('the interesting result is not that these are dangerous - it is how little the severity of a mistake has to do with whether anything reports it. The double free was caught instantly and by name. The 4 KiB write into a 64-byte allocation was not reported by the write, not by the free of the corrupted chunk, and not by sixty-four allocations afterwards: the worst of these is the one nothing notices.');
        $out->note('read-after-free also finished quietly, returning whatever the allocator had done with those bytes since. In a longer-lived process those bytes belong to something else by then, and the read is somebody else\'s data - which is the shape of a real vulnerability rather than a crash.');
        $out->note('none of it is visible from PHP. There is no exception to catch, no error handler, and no diagnostic beyond a line glibc writes to stderr on its way out - which is why FfiBuffer validates before the access rather than trying to recover after one.');
        $out->note('and why every check it performs is a check on a number, not on the memory: the length it compares against lives in the PHP object, because the allocation itself has never known how big it is.');
        $out->context();
    },
);
