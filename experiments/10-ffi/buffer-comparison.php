...
return new Experiment(
    name: 'ffi:comparison',
    description: 'PHP string, PHP array, FFI buffer and mapped file, side by side',
    supports: ['size'],
    run: static function (Options $options, Output $out): void {
        $out->heading('four ways to hold 16 MiB, measured the same way');
         ...
         * needs the reporter and the output, and reaching for either through
         * a global is exactly the hidden state this phase removed.
         *
        ...
        $out->note('release tells the last part of the story. free() and munmap() return the memory to the OS immediately; the PHP containers return it to the allocator, which keeps the arenas - the PHP column goes back to zero and the RSS column does not.');
        $out->context();
    },
);
