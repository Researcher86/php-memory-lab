...
return new Experiment(
    ...
    description: 'json, serialize, csv, packed binary and a raw string',
    supports: ['elements'],
    run: static function (Options $options, Output $out): void {
        $out->heading('serialization: what a message costs before it reaches the socket');
         ...
         * against.
         */
        $rowCount = $options->elements(1_000);
        $records = [];
        ...
        $out->note('csv and binary round-trip only because their decoders hard-code which column is an int and which is a bool; the payloads themselves say nothing about types, so the agreement lives in two files that have to be changed together.');
        $out->note('smallest on the wire is not fastest in PHP: the packed format halves json\'s bytes and is the slowest to decode, because unpacking field by field runs in userland while json_decode() runs one C call over the whole payload. Fewer bytes are worth paying for when the wire is the bottleneck - a network, a full socket buffer - and not when it is not.');
        $out->context();
    },
);
