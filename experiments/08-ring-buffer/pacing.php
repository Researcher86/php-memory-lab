...
experiment_start('full, empty, and the cost of asking again');
...
experiment_note('the producer ran at the consumer\'s rate without either of them agreeing to it. That is backpressure, and an 8-slot buffer is the entire mechanism.');
...
experiment_note('polling is a dial between latency and CPU, and there is no setting that is good at both. A socket does not have this dial because the kernel wakes the reader exactly when there is something to read - which is the feature being given up in exchange for the copy that shared memory saves.');
experiment_note('a real system closes the gap with something to be woken on: an eventfd, a futex, a semaphore used as a signal rather than a mutex. Each of those is a syscall per message again, which is where the socket already was.');
experiment_context();
