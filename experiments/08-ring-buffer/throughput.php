...
experiment_start('ring buffer against the socket it replaces');
...
experiment_note('the shared memory is not the slow part. The same layout without the semaphore is the fastest row in the table - it moves the payload once, where the socket copies it into the kernel buffer and out again.');
experiment_note('the lock is. One acquire and one release per message, between two processes that always want it at the same time, means most acquires find it held - and an acquire that waits is a sleep and a wake, tens of microseconds against the ~8 us the operation itself costs.');
experiment_note('which is the honest summary of this phase: shared memory buys a copy, and the synchronization needed to make it safe costs more than the copy. The socket wins because the kernel does the synchronizing as a side effect of moving the bytes, and charges once for both.');
experiment_note('the ways out are all real and all cost something: batch many messages per lock, use a lock-free single-producer/single-consumer design and give up crash detection, or keep the lock and accept the rate. None of them is free, and the last column is why.');
experiment_context();
